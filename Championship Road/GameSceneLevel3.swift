...
//  Championship Road
//
//  Created by Kaan Odabaş on 4.08.2023.
//
...
class GameSceneLevel3: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        engel = childNode(withName: "engel") as! SKSpriteNode
        let engelTexture = SKTexture(imageNamed: "engel")
        let engelSize = CGSize(width: engelTexture.size().width / 2.2, height: engelTexture.size().height*2)
        engel.physicsBody = SKPhysicsBody(rectangleOf: engelSize)
        engel.physicsBody?.affectedByGravity = false
        engel.physicsBody?.isDynamic = false
        engel.physicsBody?.collisionBitMask = ColliderType.Engel.rawValue
        ...
        ball.physicsBody = SKPhysicsBody(circleOfRadius: ballTexture.size().height/3.3)
        ...
        lever1 = childNode(withName: "lever1") as! SKSpriteNode
        let leverTexture = SKTexture(imageNamed: "lever1")
        let leverSize = CGSize(width: leverTexture.size().width/12, height: leverTexture.size().height/8)
        lever1.physicsBody = SKPhysicsBody(rectangleOf: leverSize)
        lever1.physicsBody?.isDynamic = false
        lever1.physicsBody?.mass = 0.065
        lever1.physicsBody?.allowsRotation = false
        lever1.physicsBody?.collisionBitMask = ColliderType.Ball.rawValue
        
        lever2 = childNode(withName: "lever2") as! SKSpriteNode
        lever2.physicsBody = SKPhysicsBody(rectangleOf: leverSize)
        lever2.physicsBody?.isDynamic = false
        lever2.physicsBody?.mass = 0.065
        lever2.physicsBody?.allowsRotation = false
        lever2.isHidden = true
        ...
        menuButton.size = CGSize(width: 260, height: 115)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if contact.bodyA.collisionBitMask == ColliderType.Ball.rawValue || contact.bodyB.collisionBitMask == ColliderType.Ball.rawValue {
            ...
            if (contact.bodyA.node == ball && contact.bodyB.node == lever1) || (contact.bodyA.node == lever1 && contact.bodyB.node == ball) {
                lever1.isHidden = true
                lever2.isHidden = false
                engel.isHidden = true
            }
            ...
            if contact.bodyA.node == cup || contact.bodyB.node == cup {
                if enemy1Health == 0 && enemy3Health == 0 {
                    playSound(name: "ucl")
                    let win = SKLabelNode(text: "Game Completed!")
                    ...
                    lever1.isHidden = false
                    lever2.isHidden = true
                    engel.isHidden = false
                }
            }
    ...
    */
                    ...
                    for node in touchNodes {
                        if let sprite = node as? SKSpriteNode {
                            if sprite == ball {
                                ...
                                cup.position = cupPosition2!
                                if engel.isHidden == true {
                                    engel.physicsBody?.categoryBitMask = 0
                                    engel.physicsBody?.collisionBitMask = 0
                                    engel.physicsBody?.contactTestBitMask = 0
                                }
                                if enemy1Health == 0 {
                                    enemy1.isHidden = true
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if let ballPhysicsBody = ball.physicsBody {
            if ballPhysicsBody.velocity.dx <= 0.02 && ballPhysicsBody.velocity.dy <= 0.02 && ballPhysicsBody.angularVelocity <= 0.6 && gameStarted == true {
    ...
    func moveEnemies() {
        let moveRightAction = SKAction.moveBy(x: 250, y: 0, duration: 0.6) // Sağa hareket
        let moveLeftAction = SKAction.moveBy(x: -250, y: 0, duration: 0.6) // Sola hareket
        let moveUpAction = SKAction.moveBy(x: 0, y: 150, duration: 0.10) // Yukarı hareket
        let moveDownAction = SKAction.moveBy(x: 0, y: -150, duration: 0.20) //Aşağı hareket


        // Düşmanları sağa ve sola hareket ettirme işlemleri
        let moveRightSequence = SKAction.sequence([moveRightAction, SKAction.wait(forDuration: 0.25), moveLeftAction])
        let moveUpSequence = SKAction.sequence([moveUpAction, SKAction.wait(forDuration: 0.15),moveLeftAction,moveDownAction,moveRightAction])

        // Düşmanlara hareket eylemlerini sürekli tekrar etme
        enemy1.run(SKAction.repeatForever(moveUpSequence))
        enemy3.run(SKAction.repeatForever(moveRightSequence))
        
        
        enemy1HealthBar.run(SKAction.repeatForever(moveUpSequence))
    ...
    func animateBallToIcardi() {
        ...
        icardi.run(moveAction) {
            ...
            let moveCameraAction = SKAction.move(to: self.icardi.position, duration: 4.0)

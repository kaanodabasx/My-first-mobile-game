...
class GameSceneLevel2: SKScene, SKPhysicsContactDelegate {
    ...
    */
                ...
                if touchedNode == nextButton {
                        let scaleAction = SKAction.sequence([
                            ...
                            SKAction.run { [weak self] in
                                self?.transitionToGameSceneLevel3()
                            }
                        ])
                        nextButton.run(scaleAction)
                        transitionToGameSceneLevel3()
                    } else {
    ...
    func moveEnemies() {
        let moveRightAction = SKAction.moveBy(x: 250, y: 0, duration: 1.0) // Sağa hareket
        let moveLeftAction = SKAction.moveBy(x: -250, y: 0, duration: 1.0) // Sola hareket
        let moveUpAction = SKAction.moveBy(x: 0, y: 120, duration: 0.15) // Yukarı hareket
        let moveDownAction = SKAction.moveBy(x: 0, y: -120, duration: 0.25) //Aşağı hareket
        ...
        let moveUpSequence = SKAction.sequence([moveUpAction, SKAction.wait(forDuration: 0.3),moveDownAction,moveLeftAction,moveRightAction])
    ...
    func transitionToGameSceneLevel3() {
        if let view = self.view as? SKView {
            ...
            if let Newscene = SKScene(fileNamed: "GameSceneLevel3") as? GameSceneLevel3 {
